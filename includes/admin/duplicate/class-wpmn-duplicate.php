...
if ( ! class_exists( 'WPMN_Media_Duplicate' ) ) :
    ...
    class WPMN_Media_Duplicate {
        ...
        public function duplicate_folder_request() {
			...
			$result = $this->duplicate_folder($folder_id);
		...
		public function duplicate_folder($folder_id, $new_parent = null) {
            ...
            // 2. Copy Post Type Meta
            $post_type = get_term_meta($folder_id, 'wpmn_post_type', true);
            if ($post_type) :
                update_term_meta($new_folder_id, 'wpmn_post_type', $post_type);
            endif;
			...
			if (!empty($direct_children) && !is_wp_error($direct_children)) :
				foreach ($direct_children as $child_id) :
					$this->duplicate_folder($child_id, $new_folder_id);
				endforeach;
			endif;
