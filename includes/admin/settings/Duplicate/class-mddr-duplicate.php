...
if ( ! class_exists( 'MDDR_Media_Duplicate' ) ) :

    /**
     * Main MDDR_Media_Duplicate Class
     *
     * @class MDDR_Media_Duplicate
     */
    class MDDR_Media_Duplicate {
        ...
        public function events_handler() {
            add_action( 'mddr_ajax_duplicate_folder', [ $this, 'duplicate_folder_request' ] );
            add_action( 'mddr_ajax_nopriv_duplicate_folder', [ $this, 'duplicate_folder_request' ] );
        }
        ...
        public function duplicate_folder($folder_id, $new_parent = null, $post_type = 'attachment') {
            ...
            $new_term = MDDR_Helper::create_folder($new_name, $new_parent);

            if (is_wp_error($new_term)) return $new_term;
            $new_folder_id = $new_term['term_id'];

            // 1. Copy Color Meta
            $color = get_term_meta($folder_id, 'mddr_color', true);
            if ($color) :
                update_term_meta($new_folder_id, 'mddr_color', $color);
            endif;

            // 2. Set Post Type Meta
            $source_post_type = get_term_meta($folder_id, 'mddr_post_type', true);
            $final_post_type  = $source_post_type ? $source_post_type : $post_type;
            
            update_term_meta($new_folder_id, 'mddr_post_type', $final_post_type);
            
            $author = get_term_meta($folder_id, 'mddr_folder_author', true);
            if ($author) update_term_meta($new_folder_id, 'mddr_folder_author', $author);
